...
static const gchar *xip_serval_cext_flags[] = {
	"RES",	/* Reserved. */
	"RES",	/* Reserved. */
	"FIN",
	"RST",
	...
	gint offset, guint8 type, guint8 length)
{
	...
	ti = proto_tree_add_uint_format(cext_tree, hf_xip_serval_ext_length,
	...
	cext_flags_tree = proto_item_add_subtree(ti, ett_xip_serval_cext_flags);
...
display_xip_serval(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	hl_ti = proto_tree_add_uint_format(xip_serval_tree, hf_xip_serval_hl,
		tvb, XSRVL_LEN, 1, xsh_len, "Header Length: %u bytes", xsh_len);
	 ...
	 * packet is malformed.
	 *
	 * Could also use val_to_str_const(), but we would need to do a
	 * string comparison to check whether to add info anyway.
	 */
	protocol = tvb_get_guint8(tvb, XSRVL_PRO);
	proto_name = try_val_to_str(protocol, xip_serval_proto_vals);
	proto_tree_add_uint_format(xip_serval_tree, hf_xip_serval_proto, tvb,
		XSRVL_PRO, 1, protocol, "Protocol: %u (%s)", protocol,
	...
	case IP_PROTO_TCP: {
		...
		offset += call_dissector(tcp_handle, next_tvb, pinfo, tree);
		break;
	}
	case IP_PROTO_UDP:
		/* The UDP header is always 8 bytes. */
		next_tvb = tvb_new_subset(tvb, offset, 8, 8);
		offset += call_dissector(udp_handle, next_tvb, pinfo, tree);
		break;
	default:
		break;
	}
...
proto_register_xip_serval(void)
{
	static hf_register_info hf[] = {
		...
		{ "SYN", "xip_serval.cext_syn", FT_BOOLEAN, BASE_NONE,
		  TFS(&tfs_set_notset), 0x0, NULL, HFILL }},

		{ &hf_xip_serval_cext_rsyn,
		{ "RSYN", "xip_serval.cext_rsyn", FT_BOOLEAN, BASE_NONE,
		  TFS(&tfs_set_notset), 0x0, NULL, HFILL }},

		{ &hf_xip_serval_cext_ack,
		{ "ACK", "xip_serval.cext_ack", FT_BOOLEAN, BASE_NONE,
		  TFS(&tfs_set_notset), 0x0, NULL, HFILL }},

		{ &hf_xip_serval_cext_nack,
		{ "NACK", "xip_serval.cext_nack", FT_BOOLEAN, BASE_NONE,
		  TFS(&tfs_set_notset), 0x0, NULL, HFILL }},

		{ &hf_xip_serval_cext_rst,
		{ "RST", "xip_serval.cext_rst", FT_BOOLEAN, BASE_NONE,
		  TFS(&tfs_set_notset), 0x0, NULL, HFILL }},

		{ &hf_xip_serval_cext_fin,
		{ "FIN", "xip_serval.cext_fin", FT_BOOLEAN, BASE_NONE,
		  TFS(&tfs_set_notset), 0x0, NULL, HFILL }},
		...
		{ "Nonce", "xip_serval.cext_nonce", FT_STRING,
